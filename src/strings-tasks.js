...
function encodeToRot13(str) {
  ...
  return str.replace(/[A-Z]/gi, (char) => rot13Alpha[alphabet.indexOf(char)]);
}
...
function getCardId(value) {
  ...
  return value[0].match(/[\d]/)
    ? Number(value.slice(0, -1)) + deckValues.get(value.slice(-1)) - 1
    : deckValues.get(value[0]) + deckValues.get(value[1]) - 1;
}
